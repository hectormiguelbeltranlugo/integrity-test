...
//const app = express();
//
//// Log every request (so you’ll see exactly what the service receives)
//app.use((req, _res, next) => {
//  console.log(`[REQ] ${req.method} ${req.originalUrl} headers=${JSON.stringify(req.headers)}`);
//  next();
//});
//
//// Handle ALL methods for /healthz (GET/HEAD/OPTIONS/etc)
//app.all('/healthz', (req, res) => {
//  console.log('[ROUTE] /healthz hit via', req.method);
//  res.set('Cache-Control', 'no-store').json({ ok: true });
//});
//
//app.all('/h', (req, res) => {
...
//app.listen(port, '0.0.0.0', () => console.log(`[BOOT] Health-only listening on :${port}`));
///**/
...
const auth = new GoogleAuth({
  scopes: ['https://www.googleapis.com/auth/playintegrity']
                        ...
                        app.get('/healthz-v1', (_req, res) => {
                          console.log("[ROUTE] GET /healthz-v1 hit");
                          res.json({ ok: true });
                        });
                        ...
                        app.get('/', (_req, res) => res.json({ ok: true, hint: 'Use /healthz-v1 or POST /decode' }));
